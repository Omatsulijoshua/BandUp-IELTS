...
class _SpeakingPracticeScreenState extends ConsumerState<SpeakingPracticeScreen> {
  ...
  /// Dynamically resolves the questions based on the currently selected test
    ...
    // Part 1: Questions 1-4 (Holidays)
    {
      ...
      'part': 1,
      'transcript':
          'I generally prefer spending my holidays with my family. I find that it is a wonderful opportunity to reconnect with my relatives and strengthen our bonds, as we are often too busy with work or studies to spend quality time together during the rest of the year.',
    },
    {
      ...
      'part': 1,
      'transcript':
          'I typically prefer staying in boutique hotels or guesthouses. I enjoy these types of accommodations because they often provide a more personalized experience and a unique, local atmosphere that large chain hotels sometimes lack.',
    },
    {
      ...
      'part': 1,
      'transcript':
          'For my next holiday, I am planning to visit a coastal town in the south. I have already booked my flight and I am currently researching some interesting historical sites and local restaurants that I would like to explore while I am there.',
    },
    {
      ...
      'part': 1,
      'transcript':
          'Yes, my city is definitely a great place for tourists. It offers a perfect blend of modern architecture and historical landmarks, and the local cuisine is absolutely world-class, which makes it very attractive for visitors who enjoy cultural exploration.',
    },
    ...
    // Part 3: Questions 6-11 (Family Celebrations & Relationships)
    {
      ...
      'part': 3,
      'transcript':
          'In my country, families typically celebrate together during major national holidays like Tet or Lunar New Year. Additionally, significant life events such as weddings, graduations, and milestone birthdays serve as primary occasions for family gatherings.',
    },
    {
      ...
      'part': 3,
      'transcript':
          'Multi-generational gatherings often occur during traditional festivals or annual reunions. However, in urban areas, these events are becoming less frequent due to busy work schedules, so they usually happen only once or twice a year.',
    },
    {
      ...
      'part': 3,
      'transcript':
          'Some individuals may find family occasions stressful due to the pressure to conform to social expectations or strained relationships with relatives. Additionally, the need to engage in small talk with people they rarely see can be exhausting for introverts.',
    },
    {
      ...
      'part': 3,
      'transcript':
          'I believe it is highly beneficial for parents to assist their children with schoolwork as it fosters a supportive learning environment. It allows parents to monitor academic progress and helps children develop a deeper understanding of complex subjects.',
    },
    {
      ...
      'part': 3,
      'transcript':
          'Eating together is crucial as it strengthens family bonds and provides a space for meaningful communication. In today\'s fast-paced world, sharing at least one meal a day acts as a vital ritual for maintaining emotional connection and family stability.',
    },
    {
      ...
      'part': 3,
      'transcript':
          'Yes, I strongly believe that sharing household chores is essential for promoting equality and responsibility within a family. When every member contributes, it lightens the burden on one individual and teaches children the importance of cooperation and life skills.',
    },
  ];
  ...
  void _startExaminerSession() async {
    ...
    // ONLY for legacy combined multi-question audio files (e.g., 1-4.mp3, 6-11.mp3)
    _audioPositionSubscription?.cancel();
    _audioPositionSubscription = _audioPlayer!.onPositionChanged.listen((position) {
      ...
      if (_currentScreen == 'EXAMINER_SESSION' && _isExaminerSpeaking && !_isAudioSeeking) {
        ...
        final String rawAsset = (currentQ['audioAsset'] as String?) ?? '';
        final bool isMultiQuestionFile = rawAsset.contains('-') || rawAsset.contains('1-4') || rawAsset.contains('6-11');

        if (isMultiQuestionFile) {
          final double startSec = (currentQ['start'] as num?)?.toDouble() ?? 0.0;
          final double promptEndSec = (currentQ['promptEnd'] as num?)?.toDouble() ?? 0.0;
          final int posMs = position.inMilliseconds;
          final int startMs = (startSec * 1000).toInt();
          final int endMs = (promptEndSec * 1000).toInt();

          if (endMs > 0 && posMs >= endMs && posMs >= startMs) {
            _audioPlayer!.pause();
            _examinerSpeakingTimer?.cancel();
            setState(() {
    ...
    // Completion listener when question audio finishes playing (natural end for all individual q{N}.mp3 files)
    _audioPlayerCompleteSubscription?.cancel();
  ...
  void _playCurrentQuestion() {
    ...
    final double startSec = (currentQ['start'] as num?)?.toDouble() ?? 0.0;
    final double promptEndSec = (currentQ['promptEnd'] as num?)?.toDouble() ?? 0.0;
    final double durationSec = (currentQ['duration'] as num?)?.toDouble() ?? 
        ((promptEndSec > startSec) ? (promptEndSec - startSec) : 5.0);
    final int promptDurationMs = (durationSec * 1000).toInt();

    // Safety watchdog timer: generous buffer (+5000ms, min 15s) so it NEVER cuts off audio while examiner speaks
    _examinerSpeakingTimer?.cancel();
    final int watchdogTimeoutMs = (promptDurationMs + 5000).clamp(15000, 35000);
    _examinerSpeakingTimer = Timer(Duration(milliseconds: watchdogTimeoutMs), () {
      if (mounted && _currentScreen == 'EXAMINER_SESSION' && _isExaminerSpeaking) {
        _audioPlayer?.pause();
        _flutterTts.stop();
        setState(() {
          _isExaminerSpeaking = false;
